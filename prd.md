...
## 7. 세부 개발 순서 (Implementation Order)

안정적이고 단계적인 개발 및 검증을 위해 아래 순서로 기능 구현을 진행합니다. 각 단계의 기능이 완성되면 단위 테스트를 거쳐 다음 단계로 이행합니다.

### 1단계: 메인 화면 구성 (UI/UX 기초 수립)
* **CSS 변수 선언**: `design.md`에 기초한 다크 모드 색상 체계, 네온 트렌드 컬러, 폰트 규격을 스타일시트에 이식합니다.
* **뼈대 레이아웃**: 대시보드 타이틀, 시장 요약 그리드, 시세 테이블, 검색창 및 토글 컨트롤 버튼들의 전역 레이아웃 프레임을 마크업합니다.
* **정적 렌더링 검증**: 더미 데이터를 활용하여 테이블의 각 컬럼(로고, 이름, 가격, 스파크라인 슬롯 등)이 정적으로 왜곡 없이 표시되는지 확인합니다.

### 2단계: API 연동 및 데이터 수집
* **환경변수 세팅**: `.env` 및 `.env.example` 파일을 작성하여 CoinGecko API Key 보안 관리를 설정합니다.
* **API 호출 서비스 모듈**: `/coins/markets` 엔드포인트에 시가총액 상위 50개 코인 정보 및 스파크라인 데이터를 요청하는 비동기 함수를 구현합니다.
* **자동/수동 갱신 루프**: 1분마다 주기적으로 API를 자동 호출(Polling)하는 타이머를 작동시키고, 새로고침 버튼 로딩 애니메이션을 바인딩합니다.
* **전역 에러 핸들링**: 네트워크 장애 및 429 Rate Limit 에러 발생 시 대시보드 테이블을 에러 전용 UI로 대체하고 "다시 시도" 버튼을 활성화하는 예외 처리를 적용합니다.

### 3단계: 데이터 가공 및 시장 분석
* **시장 상태 요약 연산**: API에서 반환한 50개 코인의 24시간 변동률 양수/음수 개수 비율을 바탕으로 상승장(Bullish)/하락장(Bearish) 상태를 진단하여 요약 카드에 반영합니다.
* **TOP 5 랭킹 정렬**: 24시간 변동률 상위 5개(상승 TOP 5)와 하위 5개(하락 TOP 5) 코인을 추출하는 정렬 가공 로직을 구현합니다.
* **인라인 스파크라인 렌더링**: 각 코인의 7일 가격 변동 데이터 배열을 활용해, 등락 추이에 맞는 색상(상승=빨강, 하락=파랑)으로 SVG 선 그래프를 생성하는 모듈을 엮어 테이블에 바인딩합니다.

### 4단계: 사용자 기능 및 인터랙션 구현
* **클라이언트 사이드 검색**: 검색창 입력 이벤트 감지 즉시, 수신한 50개 데이터 풀 내에서 이름/심볼 문자열 매칭 필터링을 수행합니다. (API 재호출 없음)
* **동적 컬럼 정렬**: 가격, 등락률(1h/24h/7d), 시가총액 헤더 클릭 시 오름차순/내림차순 정렬 기능을 구현하고 정렬 방향 아이콘을 갱신합니다.
* **LocalStorage 관심 코인**: 코인 행 좌측의 별표(★) 클릭 시 LocalStorage 내 배열을 토글 제어하고, '즐겨찾기 목록만 보기' 스위치 켬 시 해당 코인들만 노출되도록 필터링합니다.
* **상세 차트 모달**: 코인 클릭 시 상세 모달을 열고, `/coins/{id}/market_chart` API 연동을 통해 Chart.js 기반의 그라데이션 선 차트와 인터랙티브 툴팁 기능을 구동합니다.
